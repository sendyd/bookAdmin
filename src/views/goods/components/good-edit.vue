...
import { getGoodsInfo , updateGoodsInfo} from '@/api/goods'
...
const init = () => {
  getGoodsInfo(route.query.id).then(res=>{
    // console.log(res);
    state.ruleForm.category_id   = res.category_id
    state.ruleForm.title   = res.title
    state.ruleForm.description   = res.description
    state.ruleForm.price   = res.price
    state.ruleForm.stock   = res.stock
    state.ruleForm.cover   = res.cover
    state.ruleForm.pics   = res.pics
    state.ruleForm.details   = res.details
  })
}
...
const submitForm = () => {
 ruleForm.value.validate(valid=>{
   if(valid) {
     updateGoodsInfo(route.query.id , state.ruleForm).then(res=>{
      //  console.log(res);
       if(res.status === 204) {
